...
function requestCab() {
//    alert("I'm here to request for a cab");
    ...
    other_info = getById("other_info");
    
    timeFrame = time_frame + " " + time_label;
    
    if (current_location && destination && pickup_time) {
        if (xhttp.readyState == 4 && xhttp.status == 200) {
            var resp = xhttp.responseText;
            if (resp === "successful") {
                var current_url = window.location.href;
        ...
        var url = 'http://cabkonekt.appspot.com/request?current_location=' + current_location + '&destination=' + destination + "&time=" + pickup_time + "&timeframe=" + timeframe + "&phone_number=" + number;
        ...
        var error = "All data are required please";
        var error_el = mgetById("request_error").style.display = "block";
